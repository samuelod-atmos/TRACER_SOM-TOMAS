  ...
  for file in files:
    ...
    for i in np.arange(2,len(saprc_spname)-NACT-1):
      ...
      if saprc_spname[i][:7] == som_grid:
        ...
        #print(np.mean(Mk[low_indx:up_indx,i],axis=0))
    ...
    c1 = axes.pcolormesh(Cs,Os,OC_grid_a/(OC_grid_a+OC_grid_g))
    #c1 = axes.pcolormesh(Cs,Os,OC_grid)
    
    plt.title('%s Gas/Aerosol'%som_grid)
    plt.ylabel('Oxygen #')
    plt.xlabel('Carbon #')
    #fig.colorbar(c1,format=mpl.ticker.FormatStrFormatter('$10^{%2.1f}$'),pad=0.01, shrink=0.6, aspect=14, label='Gas/Aer. Ratio')
    fig.colorbar(c1,format=mpl.ticker.FormatStrFormatter('%2.1f'),pad=0.01, shrink=0.6, aspect=14, label='Frac. in Aerosol')
    #c1.set_clim(0,8)
    c1.set_clim(0.001,1)
    plt.show()
    ...
    if save_png==True:
      ...
      fig.savefig('NPF5_%s_%s_OCgrid_ratio.png'%(file[11:-11],som_grid),bbox_inches='tight')

...
n=3;m=3
Y = (number_conc[:,:])/BinsE[:]
small = np.sum(Y[:,10:15],axis=1)
med = np.sum(Y[:,15:19],axis=1)
accume= np.sum(Y[:,19:22],axis=1)
large = np.sum(Y[:,22:26],axis=1)
...
axes.set_ylabel('#/cm^3',fontsize=22)
axes.set_xlabel('Date',fontsize=22)
axes.xaxis.set_major_formatter(mdates.DateFormatter('%m/%d'))

plt.plot(x,small,label='13-30 nm')
plt.plot(x,med,label='30-80 nm')
plt.plot(x,accume,label='80-200 nm')
plt.plot(x,large,label='200-400 nm')

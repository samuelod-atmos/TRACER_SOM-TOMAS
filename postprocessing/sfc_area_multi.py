        ...
        for org_nuc in organic_nuc:
          for inorg_nuc in inorganic_nuc:
            for OH_scale in OH_multi:
              ...
              c1 = axes.plot(x,Surf2,color=color,zorder=z,label=label)
              
              #r_square1 = pearsonr(totalN,Y1)[0]
              r_square1 = Pearson_correlation(Surf,Surf2)
              mae1 = np.mean(abs(Surf2 - Surf))
              mse1 = (sum((Surf-Surf2)**2.0))/len(Surf)

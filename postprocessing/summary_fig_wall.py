...
#=============================================================================================
'''
spname_file = '%s/20220801_%s_vwl1_pwl1_hr1.44e+02_nh35000_orgfn1_inorg1_db%s_spec.dat'%(output_dir,identify,db[0])
...
gcfile = '%s/20220801_%s_vwl1_pwl1_hr1.44e+02_nh35000_orgfn1_inorg1_db%s_saprcgc.dat'%(output_dir,identify,db[0])
...
axes[2].plot(x,benz,color='y',label='Benzene')
axes[2].plot(x,isop,color='g',label='Isoprene')
axes[2].plot(x,tolu,color='brown',label='Toluene')
axes[2].plot(x,terp,color='m',label='Terpenes')
axes[2].plot(x,trimeth,color='c',label='Trimethylbenzene')
axes[2].plot(x,xylene,color='lime',label='M-Xylene')
axes[1].plot(x,so2,color='orange',linestyle='--',label='SO2')
axes[1].set_ylabel('[ppbv]',color='orange')
axes[1].legend(loc=4,prop={'size': 6})

axes[2].set_ylabel('[ppbv]')
axes[2].legend(loc=2,prop={'size': 5})
...
rh_fid = open('../inputs/20220801_%s_RH'%identify,'r')
...
axes[2].plot(x,RH*100.,color='green')
axes[2].set_xlabel('Date')
axes[2].set_ylabel('RH [%]',color='green')


T_fid = open('../inputs/20220801_%s_Temp'%identify,'r')
...
aemass_wall_file = '%s/20220801_%s_vwl1_pwl1_hr1.44e+02_nh35000_orgfn%s_inorg%s_db%s_wallmk.dat'%(output_dir,identify,orgnuc,inorgnuc,db[0])
...
axes[3].plot(x,total_org,color='green',label='Total Org.')
#plt.plot(x/360,nonvol,label='Non-Vol.')
axes[3].plot(x,so4,color='r',label='Sulfate')

axes[3].set_ylabel('Aerosol Mass \n $ \mu g $ $ m^{-3} $')
axes[3].set_yscale('log')
axes[3].legend(loc=4)
axes[3].set_ylim(0.00001,10.0)
axes[3].set_yticks([1e-5,1e-3,1e-1])


aemass_file = '%s/20220801_%s_vwl1_pwl1_hr1.44e+02_nh35000_orgfn%s_inorg%s_db%s_aemass.dat'%(output_dir,identify,orgnuc,inorgnuc,db[0])
...
axes[3].plot(x,total_org,color='green',linestyle='--',label='Total Org.')
axes[3].plot(x,so4,color='r',linestyle='--',label='Sulfate')
axes[3].grid(True)
...
gc_wall_file = '%s/20220801_%s_vwl1_pwl1_hr1.44e+02_nh35000_orgfn%s_inorg%s_db%s_wallgc.dat'%(output_dir,identify,orgnuc,inorgnuc,db[0])
# C* bin bounds 
l1,u1 = 1e-2,1e-1
l2,u2 = 1e-1,1e0
l3,u3 = 1e0,1e1
...
axes[4].plot(x,cstar_n2_n1,color='b',label='C*=%s - %s'%(str(l1),str(u1)))
axes[4].plot(x,cstar_n1_0, color='g',label='C*=%s - %s'%(str(l2),str(u2)))
axes[4].plot(x,cstar_0_1,  color='r',label='C*=%s - %s'%(str(l3),str(u3)))
axes[4].plot(x,cstar_1_2,  color='c',label='C*=%s-%s'%(str(l4),str(u4)))
axes[4].plot(x,cstar_2_3,  color='m',label='C*=%s-%s'%(str(l5),str(u5)))
axes[4].plot(x,cstar_3_4,  color='y',label='C*=%s-%s'%(str(l6),str(u6)))

#plt.title('Vapor Wall Losses')
#plt.xlabel('Date')
#axes[4].set_ylabel('ug/m^3',fontsize=12)
#ax.xaxis.set_major_formatter(mdates.DateFormatter('%m/%d'))
#plt.yscale('log')
#plt.ylim(0,2)
#axes[4].legend(loc='best',fontsize=5)
...
axes[4].plot(x,cstar_n2_n1,color='b',linestyle='--') #,label='C*=%s - %s'%(str(l1),str(u1)))
axes[4].plot(x,cstar_n1_0, color='g',linestyle='--') #,label='C*=%s - %s'%(str(l2),str(u2)))
axes[4].plot(x,cstar_0_1,  color='r',linestyle='--') #,label='C*=%s - %s'%(str(l3),str(u3)))
axes[4].plot(x,cstar_1_2,  color='c',linestyle='--') #,label='C*=%s-%s'%(str(l4),str(u4)))
axes[4].plot(x,cstar_2_3,  color='m',linestyle='--') #,label='C*=%s-%s'%(str(l5),str(u5)))
axes[4].plot(x,cstar_3_4,  color='y',linestyle='--') #,label='C*=%s-%s'%(str(l6),str(u6)))

#plt.title('Vapor Wall Losses')
#plt.xlabel('Date')
#axes[4].set_ylabel('ug/m^3',fontsize=12)
axes[4].set_ylabel('Gas Phase \n $ \mu g $ $ m^{-3} $')
#ax.xaxis.set_major_formatter(mdates.DateFormatter('%m/%d'))
axes[4].set_yscale('log')
axes[4].set_ylim(0.0005,70)
axes[4].legend(loc=4,fontsize=8)
axes[4].grid(True)
...
plt.show()
if save_png==True:
  fig.savefig('%s_wall_summary_fig.png'%rname[len(output_dir)+1:-11],bbox_inches='tight')

...
C-----VARIABLE DECLARATIONS------------------------------------------
      integer i,j,l,n,k,ttt,v      !counters
...
C--------------------------------------------------------------------------------
      ...
      DOUBLE PRECISION, ALLOCATABLE :: KZ_lay(:),rh_lay(:)
...
C      print*,'sfctemp=', sfctemp
      ...
      DO I=1,nlayers 
        write(*,*) 'Enter RH for layers'
        read(*,'(8F8.5)') rh_lay(I)
C      rh = 0.2
        print*,'rh_lay=', rh_lay(I)
      ENDDO
...
C       sigma_bg2 = sigma_bg2_lay(v)
       NH3MOLEC_CM3 = NH3_ppt*1.E-12*pres*6.022E23/temp/R*1.e-6
...
C           print*,'CONST(6):',CONST(6)
           dHOM = SAPRCGC(INDEX_VOC(I))*kOH(I)
     &      *fHOM(I)*CONST(6)
           HOM = HOM + dHOM
         END DO
C         HOM = HOM*1e-6*PRES/R/TEMP*6.022E23
         HOM = HOM*1e-6*PRES/R/TEMP*300.0/1000.0  ! HOM in kg
...
C--------------------------------------------------------
        V1 = (pres_lay(v)*boxvol_lay(v)/10000000.0*sfctemp)/
     &    (temp_lay(v)*sfcpres)
        V2 = (pres_lay(v+1)*boxvol_lay(v+1)/10000000.0*sfctemp)/
     &    (temp_lay(v+1)*sfcpres)
C        V1 = (pres_lay(v)*boxvol_lay(v)*sfctemp)/
C     &    (temp_lay(v)*sfcpres)
C        V2 = (pres_lay(v+1)*boxvol_lay(v+1)*sfctemp)/
C     &    (temp_lay(v+1)*sfcpres)
        print*,'V1 =',V1,'V2 =',V2
        print*,'Z_lay(v+1)-Z_lay(v)=',Z_lay(v+1)-Z_lay(v)
C Vertical mixing of gas-phase species
C--------------------------------------------------------
        DO I=1,icomp-1
          Jdiff = KZ_lay(v)*((Gc_lay(I,v)/V1)-(Gc_lay(I,v+1)/V2))/
     &      ((Z_lay(v+1)-Z_lay(v))**2)
C          print*,'Jdiff in GC =',Jdiff
C          print*,'(Gc_lay(I,v)/V1)-(Gc_lay(I,v+1)/V2) =',
C     &         (Gc_lay(I,v)/V1)-(Gc_lay(I,v+1)/V2)
C          print*,'(Z_lay(v+1)-Z_lay(v))**2 =',
C     &         (Z_lay(v+1)-Z_lay(v))**2 
          dCdt = Jdiff !/(Z_lay(v+1)-Z_lay(v))
C          print*,'Gc_lay(I,v) =',Gc_lay(I,v)
C          print*,'Gc_lay(I,v+1) =',Gc_lay(I,v+1)

          IF ((ABS(dCdt).lt.Gc_lay(I,v)).and.(ABS(dCdt).lt.
     &        Gc_lay(I,v+1))) THEN
            IF (dCdt.gt.0.d0) THEN
              Gc_lay(I,v) = Gc_lay(I,v)-dCdt*aadt*V1
              Gc_lay(I,v+1) = Gc_lay(I,v+1)+dCdt*aadt*V1
C              print*,'dCdt*aadt*V1 =',dCdt*aadt*V1
            ENDIF
            IF (dCdt.lt.0.d0) THEN
              Gc_lay(I,v) = Gc_lay(I,v)-dCdt*aadt*V2
              Gc_lay(I,v+1) = Gc_lay(I,v+1)+dCdt*aadt*V2
C              print*,'dCdt*aadt*V2 =',dCdt*aadt*V2
            ENDIF
          ENDIF
          
          IF ((ABS(dCdt).gt.Gc_lay(I,v)).or.(ABS(dCdt).gt.
     &        Gc_lay(I,v+1))) THEN
C            print*,'Trying the loop!!!'
            DO ttt=1,50
              Jdiff = KZ_lay(v)*((Gc_lay(I,v)/V1)-(Gc_lay(I,v+1)/V2))/
     &          ((Z_lay(v+1)-Z_lay(v))**2)
C              print*,'Jdiff =',Jdiff,'ttt =',ttt
              IF (Jdiff.gt.0.d0) THEN
                Gc_lay(I,v) = Gc_lay(I,v)-Jdiff*aadt*V1*0.01
                Gc_lay(I,v+1) = Gc_lay(I,v+1)+Jdiff*aadt*V1*0.01
C                print*,'Jdiff*aadt*V1 =',dCdt*Jdiff*V1
              ENDIF
              IF (Jdiff.lt.0.d0) THEN
                Gc_lay(I,v) = Gc_lay(I,v)-Jdiff*aadt*V2*0.01
                Gc_lay(I,v+1) = Gc_lay(I,v+1)+Jdiff*aadt*V2*0.01
C                print*,'Jdiff*aadt*V2 =',Jdiff*aadt*V2
              ENDIF
            ENDDO
          ENDIF
          IF (Gc_lay(I,v).lt.0.d0) THEN
            print*,'Gc_lay(I,v) less than 0.0'
            print*,'Gc_lay(I,v) =',Gc_lay(I,v)
            print*,'Jdiff =',Jdiff
            print*,'dCdt =',dCdt
            print*,'dCdt*aadt*V1 =',dCdt*aadt*V1
            print*,'Layer =',v
            print*,'I=',I
            STOP
          ENDIF
          IF (Gc_lay(I,v+1).lt.0.d0) THEN
            print*,'Gc_lay(I,v+1) less than 0.0'
            print*,'Gc_lay(I,v+1) =',Gc_lay(I,v+1)
            print*,'Jdiff =',Jdiff
            print*,'dCdt =',dCdt
            print*,'dCdt*aadt*V2 =',dCdt*aadt*V2
            print*,'Layer =',v
            print*,'I=',I
            STOP
          ENDIF
...
C--------------------------------------------------------
        DO n=1,ibins
C          print*,'Nk_lay(n,v)/V1 =',Nk_lay(n,v)/V1
C          print*,'Nk_lay(n,v+1)/V2 =',Nk_lay(n,v+1)/V2
C          print*,'bin =',n
...
C          print*,'dCdt =',dCdt
C          print*,'dCdt*aadt*V1 =',dCdt*aadt*V1
          IF ((ABS(dCdt*aadt*V1).lt.Nk_lay(n,v)).and.
     &        (ABS(dCdt*aadt*V1).lt.Nk_lay(n,v+1)).and.
     &        (ABS(dCdt*aadt*V2).lt.Nk_lay(n,v)).and.
     &        (ABS(dCdt*aadt*V2).lt.Nk_lay(n,v+1))) THEN
            IF (dCdt.gt.0.d0) THEN
              Nk_lay(n,v) = Nk_lay(n,v)-dCdt*aadt*V1
C              print*,'Nk_lay(n,v) =',Nk_lay(n,v)
              Nk_lay(n,v+1) = Nk_lay(n,v+1)+dCdt*aadt*V1
C              print*,'Nk_lay(n,va+1) =',Nk_lay(n,v+1)
              ratio = ABS(aadt*dCdt/(Nk_lay(n,v)/V1))
              DO I=1,icomp
                delmass = Mk_lay(n,I,v)*ratio
                Mk_lay(n,I,v) = Mk_lay(n,I,v)-delmass
                Mk_lay(n,I,v+1) = Mk_lay(n,I,v+1)+delmass
              ENDDO

            ELSEIF (dCdt.lt.0.d0) THEN
              Nk_lay(n,v) = Nk_lay(n,v)-dCdt*aadt*V2
C              print*,'Nk_lay(n,v) =',Nk_lay(n,v)
              Nk_lay(n,v+1) = Nk_lay(n,v+1)+dCdt*aadt*V2
C              print*,'Nk_lay(n,v+1) =',Nk_lay(n,v+1)
              ratio = ABS(aadt*dCdt/(Nk_lay(n,v+1)/V2))
              DO I=1,icomp
                delmass = Mk_lay(n,I,v+1)*ratio
                Mk_lay(n,I,v+1)=Mk_lay(n,I,v+1)-delmass
                Mk_lay(n,I,v) = Mk_lay(n,I,v) + delmass
              ENDDO
            ENDIF
          ELSE 
            print*,'Trying the loop in layer:',v,'bin:',n
            print*,'Nk_lay(n,v)/V1 =',Nk_lay(n,v)/V1
            print*,'Nk_lay(n,v+1)/V2 =',Nk_lay(n,v+1)/V2
C            Jdiff = KZ_lay(v)*((Nk_lay(n,v)/V1)-(Nk_lay(n,v+1)/V2))/
C     &      ((Z_lay(v+1)-Z_lay(v))**2)
            DO ttt=1,20
              Jdiff = KZ_lay(v)*((Nk_lay(n,v)/V1)-(Nk_lay(n,v+1)/V2))/
     &        ((Z_lay(v+1)-Z_lay(v))**2)
              print*,'Jdiff =',Jdiff,'ttt =',ttt
              IF (Jdiff.gt.0.d0) THEN
                Nk_lay(n,v) = Nk_lay(n,v)-Jdiff*aadt*V1*0.05
                Nk_lay(n,v+1) = Nk_lay(n,v+1)+Jdiff*aadt*V1*0.05
                print*,'Jdiff*aadt*V1 =',dCdt*Jdiff*V1
                ratio = ABS(0.05*Jdiff/(Nk_lay(n,v)/V1)*aadt)
                print*,'ratio =',ratio
               DO I=1,icomp
                 delmass = Mk_lay(n,I,v)*ratio
                 Mk_lay(n,I,v) = Mk_lay(n,I,v)-delmass
                 Mk_lay(n,I,v+1) = Mk_lay(n,I,v+1)+delmass
                 IF (Mk_lay(n,I,v).lt.0.d0) THEN
                   print*,'Mk_lay(n,I,v) less than 0.0, line 1627'
                   print*,'layer =',v
                   print*,'Bin =',n
                   print*,'Comp. =',I
                   STOP
                 ENDIF
                 IF (Mk_lay(n,I,v+1).lt.0.d0) THEN
                   print*,'Mk_lay(n,I,v+1) less than 0.0, line 1631'
                   print*,'layer =',v
                   print*,'Bin =',n
                   print*,'Comp. =',I
                   STOP
              ...
              ELSEIF (Jdiff.lt.0.d0) THEN
                Nk_lay(n,v) = Nk_lay(n,v)-Jdiff*aadt*V2*0.05
                Nk_lay(n,v+1) = Nk_lay(n,v+1)+Jdiff*aadt*V2*0.05
                print*,'Jdiff*aadt*V2 =',Jdiff*aadt*V2
                ratio = ABS(0.05*Jdiff/(Nk_lay(n,v+1)/V2)*aadt)
                print*,'ratio =',ratio
                DO I=1,icomp
                  delmass = Mk_lay(n,I,v+1)*ratio
                  Mk_lay(n,I,v+1)=Mk_lay(n,I,v+1)-delmass
                  Mk_lay(n,I,v) = Mk_lay(n,I,v) + delmass
                  IF (Mk_lay(n,I,v).lt.0.d0) THEN
                    print*,'Mk_lay(n,I,v) less than 0.0, line 1646'
                    print*,'layer =',v
                    print*,'Bin =',n
                    print*,'Comp. =',I
                    STOP
                  ENDIF
                  IF (Mk_lay(n,I,v+1).lt.0.d0) THEN
                    print*,'Mk_lay(n,I,v+1) less than 0.0, line 1650'
                    print*,'layer =',v
                    print*,'Bin =',n
                    print*,'Comp. =',I
                    STOP
          ...
          IF (Nk_lay(n,v).lt.0.d0) THEN
            print*,'Nk_lay(n,v) less than 0.0'
            print*,'Nk_lay(n,v) =',Nk_lay(n,v)
            print*,'Jdiff =',Jdiff
            print*,'dCdt =',dCdt
            print*,'dCdt*aadt*V1 =',dCdt*aadt*V1
            print*,'dCdt*aadt*V2 =',dCdt*aadt*V2
            STOP
          ENDIF
          IF (Nk_lay(n,v+1).lt.0.d0) THEN
            print*,'Nk_lay(n,v+1) less than 0.0'
            print*,'Nk_lay(n,v+1) =',Nk_lay(n,v+1)
            print*,'Jdiff =',Jdiff
            print*,'dCdt =',dCdt
            print*,'dCdt*aadt*V1 =',dCdt*aadt*V1
            print*,'dCdt*aadt*V2 =',dCdt*aadt*V2
          ...
          DO I=1,icomp
            IF (Mk_lay(n,I,v).lt.0.d0) THEN
              print*,'Mk_lay(n,I,v) less than 0.0, line 1678'
              print*,'layer =',v
              print*,'Bin =',n
              print*,'Comp. =',I
              STOP
            ENDIF 
            IF (Mk_lay(n,I,v+1).lt.0.d0) THEN
              print*,'Mk_lay(n,I,v+1) less than 0.0, line 1682'
              print*,'layer =',v
              print*,'Bin =',n
              print*,'Comp. =',I
              STOP
...
C            ratio = ABS(dCdt/(Nk_lay(n,v)/V1)*aadt)
C            DO I=1,icomp
...
C            ratio = ABS(dCdt/(Nk_lay(n,v+1)/V2)*aadt)
C            DO I=1,icomp

...
C     Nkf, Mkf - same as above, but final values
      ...
      SUBROUTINE homcond(Nki,Mki,mcondi,spec,Nkf,Mkf)
...
!            print*,'test3'
     ...
     &              (Mkf(k,spec)-Mki(k,spec))/ratio
            ...
            print*,'ratio =',ratio,(tot_f-tot_i)/mcond
            print*,'Nki',Nki
            print*,'Nkf',Nkf
            print*,'Mki',Mki
            print*,'Mkf',Mkf
...
Cjrp         print*,'No S conservation in homcond.f'
...
Cjrp      endif
      ...
      tot_f = 0.d0
      do k=1,ibins
         tot_i=tot_i+Mki(k,SRTORGLAST)
         tot_f=tot_f+Mkf(k,SRTORGLAST)
      enddo
      if (abs(tot_f-tot_i)/tot_i.gt.1.0D-8)then
         print*,'No N conservation in homcond.f'
         print*,'initial',tot_i
         print*,'final',tot_f
      endif

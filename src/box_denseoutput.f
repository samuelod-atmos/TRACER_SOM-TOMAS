...
!      double precision kw0,ke!dilt2, kw_turb(36) !add by Emily
      ...
      CHARACTER*120 DIRECTORY,HOM_file,VOC_FILE,FLOW_FILE,INJECT_FILE
      character*120 tauname,kwname,cglname,walname
      character*120 binfilename, O3_FILE
      character*120 WALL_GC, WALL_MK, WALL_NK
...
C     &     '_oh.dat'
     ...
     &     '_wallnk.dat'
      VOC_FILE='../../inputs/'//RUNNAME(1:14)//
     &     '_voc'
      FLOW_FILE='../../inputs/'//RUNNAME(1:14)//
     &     '_flows'
      OH_FILE='../../inputs/'//RUNNAME(1:14)//
     &     '_OH'
      INJECT_FILE='../../inputs/'//RUNNAME(1:14)//
     &     '_inject'
      O3_FILE='../../inputs/'//RUNNAME(1:14)//
     &     '_o3'
...
C-----------------------------------------------------------------------------------
      OPEN(UNIT = 43, FILE = '../../inputs/'// runname(1:14)
     & //'_Temp',STATUS = 'old')
      READ(43,'(f8.5)') temp
      print*,'Temp=',Temp

      OPEN(UNIT = 44, FILE = '../../inputs/'// runname(1:14)
     & //'_RH',STATUS = 'old')
      READ(44,'(f12.6)') RH
      print*,'RH=',RH
...
!      WRITE(38,FRMT2) SAPRCGC(1:NACT)
      ...
      DO J=1,ICOMP
         WRITE(55,4) time/3600., (Mk_wall(N,J)*1.0E9 / 
     &                boxvol * 1.0E6, N=1, IBINS)
      END DO
      CLOSE(55)
      
      OPEN(UNIT=54, FILE=WALL_NK, STATUS='OLD', ACCESS='APPEND')
      WRITE(54,4) time/3600., (Nk_wall(K)/boxvol,K=1,IBINS)
      CLOSE(54)
      
      OPEN(UNIT=53, FILE=WALL_GC, STATUS='OLD', ACCESS='APPEND')
      WRITE(53,5) time/3600., (Gc_wall(N),N=1,ICOMP-1)
      CLOSE(53)
C      Nk_lay(:,v) = Nk(:)
C      Gc_lay(:,v) = Gc(:)
C      RKZ_lay(:,v) = RKZ(:)
...
!      WRITE(38,FRMT2) SAPRCGC(1:NACT)
 ...
 4    FORMAT(41E15.6)
      ...
      ! Write wall loss concentrations to output      
 5    FORMAT(480E15.6)
      OPEN(UNIT=55, FILE=WALL_MK, STATUS='OLD', ACCESS='APPEND')
      DO J=1,ICOMP
         WRITE(55,4) time/3600., (Mk_wall(N,J)*1.0E9 / 
     &                boxvol * 1.0E6, N=1, IBINS)
      ...
      OPEN(UNIT=53, FILE=WALL_GC, STATUS='OLD', ACCESS='APPEND')
      WRITE(53,5) time/3600., (Gc_wall(N),N=1,ICOMP-1)
!      WRITE(53,5) time/3600., (Gc_wall(N+SRTORG1-1) * 1.0E+6 / ! TOMAS gas-phase SOM
!     &                      PRES * R * TEMP / MWORG(I) *

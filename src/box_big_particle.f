...
!      double precision kw0,ke!dilt2, kw_turb(36) !add by Emily
      ...
      double precision big_Nk(1), big_Mk(1,icomp) 
      double precision big_Nko(1), big_Mko(1,icomp) 
...
C     call boxinit(time,No,Dp,sigma,SRTSO4)
     ...
     &        orgfrac_bg, boxvol, time+(tstep-1)*aadt)
      ...
      big_Mk(1,SRTORGLAST) = big_Nk(1)*1400.0*3.14159/6.0*1.56**3.0

      big_Nko = big_Nk
      big_Mko = big_Mk

      print*,'kappa_avg =',kappa_avg
      print*,'big_mk shape =',shape(big_Mk)
      print*,'big_nk shape =',shape(big_Nk)
      print*,'gc shape=',shape(Gc)
      print*,'Gcout shape =',shape(Gcout)
      print*,'aadt =',aadt
      print*,'tau_p =',tau_p
      print*,'Dbk =',Dbk
      print*,'kc =',kc
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      DO N=1,IBINS
         ...
         ENDDO !721
      ENDDO    !722 
      print*,'made it to 912' 
      DO I=1,ICOMP-1
         IF (GC(I) .LT. 1d-60) THEN
            GC(I) = 0.d0
         END IF
      ENDDO
      print*,'made it to 918' 
      DO I=1,NACT
...
C         call step_wallloss(kvap_on, kpar, VWL, PWL,
C     &        Gc, Nk, Mk, 1, Temp, time, aadt,Csat_Ref) ! mWSORB = 1 ? 
         !print*,'big_Mk(1,SRTORGLAST)=',big_Mk(1,SRTORGLAST)
         !print*,'big_Nk=',big_Nk
         !print*,Gc
         !print*,big_Nko
         !print*,big_Mko
         ...
         !print*,kc
         !print*,ORG_O2C
         print*,'SUM(GC) before =',SUM(Gc(srtorg1:srtorglast))
         call big_particle(big_Nk,big_Mk,Gc,big_Nko,big_Mko,Gcout,
     &        aadt,tau_p,Dbk,kc,ORG_O2C,kappa_avg)

         print*,'SUM(GC) after =',SUM(Gcout(srtorg1:srtorglast))
         big_Nk = big_Nko
         big_Mk = big_Mko
         Gc = Gcout

...
C     Nkf, Mkf, Gcf - same as above, but final values
      ...
      SUBROUTINE big_particle(Nki,Mki,Gci,Nkf,Mkf,Gcf,dt,tau_p,
...
C-----ARGUMENT DECLARATIONS---------------------------------------------
      INTEGER big_ibins
      parameter(big_ibins=1)
      double precision Nki(1),Mki(1, icomp),Gci(icomp-1)
      double precision Nkf(1),Mkf(1, icomp),Gcf(icomp-1)
      double precision dt

C-----VARIABLE DECLARATIONS---------------------------------------------

      double precision dp(1)  !Driving force for condensation (Pa)
      double precision tau(1)          !condensation parameter (see cond.f)
      double precision atau(1)  !same as tau, but all species
      double precision atauc(1) !same as atau, but for const dp
      ...
      double precision Dpk(1)          !diameter (m) of particles in bin k
      double precision Rpk(1)          !Radius (m) of particles in bin k
      double precision density             !density (kg/m3) of particles
      integer k,j,jj,jjj,jo           !counters
      double precision tj, tk(1)  !factors used for calculating tau
...
!      double precision Sv(icomp-idiag)         !parameter used for estimating diffusivity
      double precision beta(1)              !correction for non-continuum
      double precision mp,mps(1) !particle mass (kg)
      double precision Nko(1),Mko(1, icomp)
      double precision Gco(icomp-idiag) !output of cond routine
...
cdbg      integer numcalls          !number of times cond routine is called
      double precision zeros(1) ! array of zeros
      double precision tot_n
      double precision CS       !condensation sink [s-1]
      double precision betat(1)
      double precision Mkdry    !dry mass
      double precision WR(1) !ratio of wet mass to dry mass of particle
      ...
      double precision madd(1) ! mass to add or remove from particle
      double precision scalefactor ! scale factor for Psat change by surface tension
      double precision sinkfrac(1) ! fraction of condensation sink from each bin
      ...
      double precision masscheqm ! change in mass of species to
                                        ! reach eqm
      double precision maddEQ(1) ! change in mass of species per particle
                              ! to reach eqm
      ...
      DOUBLE PRECISION FC(1)   ! Fuch's correction factor
      DOUBLE PRECISION kgk(1)  ! gas-side mass transfer coefficient, m/s
      DOUBLE PRECISION Kgkk(1) ! gas-side mass transfer coefficient, m/s
      DOUBLE PRECISION kpk(1)  !
      ...
      DOUBLE PRECISION qk(1)   ! diffuso-reactive term, unitless
      DOUBLE PRECISION coth(1) ! hyperbolic cotangent
...
C            print*, 'mfp    =', mfp
     ...
     &        (cstar(jo)/density/1.0e9))
         print*,'Kgkk =',Kgkk(k),Rpk(k),kgk(k),kpk(k),cstar(jo),Dbk
CCCCC ============================================================================================
!     AliA - calculate Kgk
...
!         sinkfrac(k)=tk(k)*Nkf(k)*mps(k)**(1.d0/3.d0)/sK
         ...
         print*,'sinkfrac =',sinkfrac,Dpk(k),Nkf(k),kgkk(k),sK
      enddo
...
!         if (-madd(k).gt.Mkf(k,j)/Nkf(k)*1.000001)then  !original 
         ...
         !   print*,'too much evaporation in big_particle!'
         !   print*,'k',k,'j',j
         !   print*,'madd',madd(k)
...
C      print*,'dp = ',dp,'pamb =',pamb
      ...
      do k=1,big_ibins
         ...
         !!! NAN CHECK !!!
         if (isnan(Nkf(k))) then
            print*,'Nk NaN in big_particle before tmcond'
            print*,'j=',j,'k=',k
            print*,'time=',time
            print*,'tau=',atau
            print*,'sK=',sK
...
C      print*,'tau=',tau
      call tmcond_big_particle(tau,xk,Mkf,Nkf,Mko,Nko,j,zeros)
...
C      print*,'SUM(Mko) after tmcond=',SUM(Mko)
      ...
      do k=1,big_ibins
         ...
         !!! NAN CHECK !!!
         if (isnan(Nkf(k))) then
            print*,'Nk NaN in big_particle after tmcond'
            print*,'j=',j,'k=',k
            print*,'time=',time
            print*,'tau=',atau
            print*,'sK=',sK
...
C      print*,'dNerr after=',dNerr
      ...
      if (abs(dNerr/Ntoto) .gt. 1.e-4) then
         write(*,*) 'ERROR in big_particle: Number not conserved'
         write(*,*) 'time=',time
         write(*,*) 'j=', j
         write(*,*) Ntoto, Ntotf
         write(*,*) (Nkf(k),k=1,big_ibins)
      endif

      do k=1,big_ibins
         if (Nko(k).lt.0.d0) then
            print*, 'Nk < 0 in big_particle'
            print*, 'k=',k,'j=',j
            print*,'Nk=',Nko(k)
            print*,'Mk=',Mko(k,:)
            stop

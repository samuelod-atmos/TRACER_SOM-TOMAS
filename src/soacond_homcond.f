...
C-----CODE--------------------------------------------------------------
      !print*,'temp in soacond:',temp
      !print*,'pres in soacond:',pres
...
C Initialize values of Nkf, Mkf, Gcf, and time
      ...
      !print*,'SUM(Nki) at beginning=',SUM(Nki)
      !print*,'SUM(Nkf) at beginning=',SUM(Nkf)
      !print*, 'SUM(MKi) =',SUM(Mki(:,:))/boxvol*1e15
...
!     basic properties; can be eliminated if defined in TOMAS prior to this
     ...
     &        + 0.283*accom*Kn + 0.75*accom) ! Change from 0.238 to 0.283 - SamO
C     &        + 0.238*accom*Kn + 0.75*accom) ! Fuchs correction, unitless, eqn (14)
...
C      if (time .lt. dt) goto 10
      ...
      print*, 'SUM(MKf) =',SUM(Mkf(:,:))/boxvol*1e15
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      SUBROUTINE HOMCOND(Nki,Mki,Nkf,Mkf,HOM,dt,tau_p,
     &     Dbk,kc,ORG_O2C2,kappa_avg2)
...
C     CHARLES:
      DOUBLE PRECISION HOM ! NOTE THE HOM HERE SHOULD BE IN [kg]

      double precision Nki(ibins), Mki(ibins, icomp), Gci(1) ! CHARLES: !Gci(icomp-1)
      double precision Nkf(ibins), Mkf(ibins, icomp), Gcf(1) ! CHARLES: !Gcf(icomp-1)
      double precision dt
...
!      double precision Sv(icomp-idiag)         !parameter used for estimating diffusivity
      double precision beta(ibins)              !correction for non-continuum
      double precision mp,mps(ibins) !particle mass (kg)
      double precision Nko(ibins), Mko(ibins, icomp), Gco(1) !CHARLES: !Gco(icomp-idiag) !output of cond routine
      double precision mi, mf  !initial and final aerosol masses (updates Gc)
      double precision tr      ! used to calculate time step limits
      double precision mc, ttr
      double precision Neps     !value below which Nk is insignificant
...
C     CHARLES:
      ...
      !print*,'Gci(1) in homcond=',Gci(1)
      do j=1,1 !CHARLES: !icomp-2
        Gco(j)=Gci(j)
      enddo


      do j=1,1 !CHARLES: !icomp-1
         Gcf(j)=Gci(j)
         Gco(j)=Gci(j) ! Added by SamO
      enddo

      print*,'Gcf(1) begin of HOMCOND:',Gcf(1)
      print*,'Gci(1) begin of HOMCOND:',Gci(1)
...
!      ms=sqrt(8.0*R*temp/(pi*mworg(jo)*1.0d-3)) !S&P2 eqn 9.2
!     CHARLES: ASSUMED MW FOR HOMS RIGHT NOW:
      ms=sqrt(8.0*R*temp/(pi*300.0*1.0d-3)) !S&P2 eqn 9.2
...
C      Di = 1.38e-5*44.0/mworg(jo) ! from SOM
C     CHARLES: ASSUMED MW:
      Di = 1.38e-5*44.0/300.0 !mworg(jo) ! from SOM
...
C      tj=2.*pi*mworg(jo)*1.0d-3/(R*temp)
C     CHARLES: ASSUMED MW:
      tj=2.*pi*300.0*1.0d-3/(R*temp)
...
!            print*, 'Rpk(k) =', Rpk(k)
!            print*, 'kpk(k) =', kpk(k)
!            print*, 'Cstar  =', cstar(jo)
!            print*, 'mworg  =', mworg(jo)
!            print*, 'mfp    =', mfp
         else
            qk(k) = Rpk(k) * (kc/Dbk)**0.5 ! diffuso-reactive term, unitless
            coth(k) = 1.0/(tanh(qk(k))) ! hyperbolic cotangent
...
!         print*,'Kgk(k) =',Kgk(k)
!         print*,'Kpk(k) =',Kpk(k)
!         print*,'Cstar(srtorglast-1) =',Cstar(SRTORGLAST-1)
!         print*,'density =',density
!         print*,'-----------------------------------------------------'
CCCCC ============================================================================================
!     AliA - calculate Kgk
...
Ckpc  adjust beta, tk, sK and alpha for all species
         beta(k)=(1.+Kn)/(1.+2.*Kn*(1.+Kn)/alpha) !S&P eqn 11.35
!         print*, 'Kgkk(k) =', Kgkk(k)
!         print*, 'beta(k),Dgk =', beta(k),Dgk
!         print*, 'Kn =',Kn, 'alpha =',alpha
!         print*, '**************************************'
...
C Set dp for nonvolatile species
      do k=1,ibins
         ...
         print*,'nonorgscale=',nonorgscale
         print*,'totphase=',totphase

C         scalefactor=exp((storg*0.2)/(8.314*temp*1200.*Dpk(k)))  !! kelvin effect?
C       print*,'scalefactor before=',scalefactor

C      CHARLES: ASSUMED MW:
C       scalefactor=exp((storg*0.001*mworg(j))/
C     & (8.314*temp*1200.*Dpk(k)))  !! kelvin effect, mw dependent
       scalefactor=exp((storg*0.001*300.0)/
     & (8.314*temp*1200.*Dpk(k)))  !! kelvin effect, mw dependent
...
C        CHARLES:
C         pamb=(Gcf(j)/mworg(jo))/(boxmass/28.9)*pres 
         pamb=(Gcf(1)/300.0)/(boxmass/28.9)*pres
...
C         print *, 'mworg=',mworg(jo)		 
                       ! ambient pressure (Pa)
         if (totphase.gt.0.d0) then
C            psat=psatorg(jo)*Mkf(k,j)/totphase*scalefactor 

C           CHARLES: FOR SIMPLICITY, ASSUME P* TO BE EFFECTIVELY ZERO:
            psat=0.d0 !psatorg(jo)*Mkf(k,j)/totphase*scalefactor 
...
C         CHARLES: BORROW NON-VOLATILE P*:
C          masseqm=pamb/(psatorg(jo)/totphase*scalefactor)         
          masseqm=pamb/(psatorg(iORG)/totphase*scalefactor)
          masscheqm=masseqm-Mkf(k,j) ! change in mass to reach eqm
          maddEQ(k)=masscheqm/Nkf(k)
          print*,'psatorg(iORG)=',psatorg(iORG),masseqm,
     &                              masscheqm,maddEQ(k)
        endif
!        print*,'-----------------------------'
!        print*,'Nkf(k)=',Nkf(k)
!        print*,'Mkf(k,j)=',Mkf(k,j)
!        print*,'massEQ(k)=',maddEQ(k)
!        print*,'masscheqm=',masscheqm
!        print*,'masseqm =',masseqm
!        print*,'dp(k)=',dp(k)
!        print*,'pamb=',pamb
!        print*,'psat=',psat
!        print*,'boxmass=',boxmass
!        print*,'psatorg(jo)=',psatorg(jo)
!        print*,'totphase=',totphase
!        print*,'scalefactor=',scalefactor
!        print*,'-----------------------------'
!         print*, j,k,dp(k),Mkf(k,j),masseqm,masscheqm,maddEQ(k),Nkf(k)
...
C               print*,'k',k,'atau2',atau(k) ! ,srth2o)
         ...
         print*,'k',k,'madd',madd(k),'atau',atau(k)
         print*,'-madd(k)-Mkf(k,j)/Nkf(k)=',-madd(k)-Mkf(k,j)/Nkf(k)
...
C     CHARLES: PARTITION HOM TO THE EXISTING NON-VOLATILE SPECIES:
C      call tmcond(tau,xk,Mkf,Nkf,Mko,Nko,j,zeros)
      print*, 'SUM(Mkf) before tmcond =',
     &         SUM(Mkf(:,SRTORGLAST))/boxvol*1e15
      print*, 'SUM(Mko) before tmcond =',
     &         SUM(Mko(:,SRTORGLAST))/boxvol*1e15
      !print*, 'tau =',tau
      !print*, 'atau =',atau
      !print*, 'xk =',xk
      !print*, 'sk =',sk
      ...
      !print*, 'zeros =',zeros
      call tmcond(tau,xk,Mkf,Nkf,Mko,Nko,SRTORGLAST,zeros)
      print*, 'SUM(Mkf) after tmcond =',
     &         SUM(Mkf(:,SRTORGLAST))/boxvol*1e15
      print*, 'SUM(Mko) after tmcond =',
     &         SUM(Mko(:,SRTORGLAST))/boxvol*1e15
...
C      print*,'dNerr after=',dNerr
     ...
     &                         /WR(k)
      ...
      do k=1,ibins
         !mi=mi+Mkf(k,j)
         !mf=mf+Mko(k,j)
         mi=mi+Mkf(k,SRTORGLAST)
         mf=mf+Mko(k,SRTORGLAST)
      enddo

C     CHARLES:
      Gcfs=Gcf(1) !Gcf(j)
C      Gcf(j)=Gcf(j)+(mi-mf)
      print*,'Gcf(1) 1775 HOMCOND:',Gcf(1)
      Gcf(1)=Gcf(1)+(mi-mf)
      print*,'Gcf(1) 1777 HOMCOND:',Gcf(1)

      print*,'mi=',mi,'mf=',mf,'mi-mf=',mi-mf

C     CHARLES MODIFIED INDEX from j to 1:
      if (Gcf(1).lt.0.d0)then
         if ((abs(Gcf(1)/mi).lt.1d-8).or.(abs(Gcf(1)).lt.1d-8))then
            Gcf(1)=0.d0
         else
            print*,'soacond Gc < 0'
            print*,'j',j
            print*,Gcfs,Gcf(1),(mi-mf),mi,mf
            print*,'cdt',cdt
            print*,'sK:',sK
            print*,'madd:',madd
            print*,'atau:',atau
      ...
      do k=1,ibins
         ...
         enddo
      enddo
      !print*, 'SUM(Mkf) in HOMCOND =',SUM(Mkf(:,:))/boxvol*1e15

C     CHARLES: COMMENTED THIS OUT
      !Update ammonia concentrations
C      call eznh3eqm(Gcf,Mkf)

      !Update water concentrations
C      call ezwatereqm(Mkf,ORG_O2C2,kappa_avg2)
...
C    CHARLES: MAP Gcf BACK TO HOM:
      print*,'Gcf(1) end of HOMCOND:',Gcf(1)
      print*,'Gcf-Gci',Gcf(1)-Gci(1),'mi-mf=',mi-mf
      print*,'Diff',(Gcf(1)-Gci(1))-(mi-mf)
      HOM = Gcf(1)

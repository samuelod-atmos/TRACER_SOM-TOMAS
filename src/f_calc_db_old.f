...
C      DOUBLE PRECISION RH              !      Relative humidity
      ...
      DOUBLE PRECISION Frac_Orgs(iORG)!(iORG) !      Fraction of each organic species
      DOUBLE PRECISION Frac_Orgs2(iORG)
      
      DOUBLE PRECISION Tg_Orgs(iORG) !(iORG)   ! [K]  Glass-transition temperature of each organic species
...
C     SKIPPING CRITERIA:
C     ==========================================================================      
      Mass_ORG = SUM(Mk(:,SRTORG1:SRTORGLAST-1))
...
C      IF (timer.LT.1800.) THEN !1600. !1800.
C         CALC_DB = 1e-18 !3.4e-19 !1e-19 !3.4e-19
C         GOTO 100
C      END IF
...
C Sam Commented this 
      ...
      Tg_Orgs(SRTORGLAST) = A + B*300.0 + C*(300.0**2.) + D*1.5 + 
     &                   E*300.0*1.5
...
C         Frac_Orgs(i) = SUM(Mk(:,xORG_1ST+i-1))/(Mass_ORG-Mass_HOMS)
      ...
      Frac_Orgs(SRTORGLAST) = SUM(Mk(:,SRTORGLAST))/Mass_ORG

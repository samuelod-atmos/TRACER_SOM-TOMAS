...
Cpja End of my additional notes
      ...
      SUBROUTINE TMCOND_BIG_PARTICLE(TAU,X,AMKD,ANKD,AMK,ANK,
     &                              CSPECIES,moxd)
...
C      include '../headers/inc_tomas_arrays.f'
      ...
      double precision DN,DM,DYI,TAU(big_ibins),XL,XU,YL,YLC,YU,YUC
      double precision TEPS,NEPS,EX2,ZERO
      double precision XI,XX,XP,YM,WTH,W1,W2,WW,AVG
      double precision VSW,VNTF(big_ibins)
      double precision TAU_L, maxtau
      double precision X(big_ibins+1),AMKD(big_ibins,icomp)
      double precision ANKD(big_ibins)
      double precision AMK(big_ibins,icomp),ANK(big_ibins)
      double precision AMKDRY(big_ibins), AMKWET(big_ibins)
      double precision WR(big_ibins)
      double precision DMDT_INT
      double precision AMKD_tot
      PARAMETER (TEPS=1.0d-40,NEPS=1.0d-20)
      PARAMETER (EX2=2.d0/3.d0,ZERO=0.0d0)
      double precision moxd(big_ibins) !moxid/Nact (win, 5/25/06)
...
Cpja Initialize X() array of particle masses based on xk()
      DO L=1,big_ibins
         ...
         X(L)= 1.5 ! SamO changed to large particle
      ENDDO
...
c         XX=X(L)/AVG
c
c         XI=.5d0 + XX*(1.5d0 - XX)
c         
c         if (XI .LT. 1.d0) then
c            write(*,*)'XI =',XI
c            !W1 will have sqrt of negative number
c            write(*,*)'ERROR: tmcond - XI<1 for bin: ',L
c            write(*,*)'lower limit is',X(L)
c            write(*,*)'AVG is ',AVG
c            write(*,*)'Nk is ', ANKD(L)
c            write(*,*)'Mk are ', (AMKD(k,j),j=1,icomp)
c            write(*,*)'Initial N and M are: ',ANKD(L),AMKDRY(L)
c            STOP
c         endif
c
c         W1 = SQRT(12.d0*(XI-1.d0))*AVG
c         W2 = MIN(X(L+1)-AVG,AVG-X(L))
c         WTH=W1*WW+W2*(1.d0-WW)
c
c         IF(WTH.GT.1.) then
c            write(*,*)'WTH>1 in cond.f, bin #',L
c            STOP
c         ENDIF
...
C--------GROW THE PARTICLES------------------------------------------
         ...
         IF (YU .GT. X(big_ibins+1) ) THEN
            YUC = YUC*X(big_ibins+1)/YU
            YU  = X(big_ibins+1)
         ENDIF
